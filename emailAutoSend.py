...
POLL_EVERY  = int(os.getenv("POLL_EVERY_SEC", "30"))   # seconds between inbox checks
...
def _write_csv(rows: list, start_dt: datetime, stop_dt: datetime) -> str:
    """
    Writes rows to a temp CSV file with date and time as separate columns.
    ...
    fieldnames = [
        "date", "time", "product", "now_weight",
        "req_weight", "final_weight", "task_state", "reason",
    ]
    with open(path, "w", newline="") as f:
        ...
        for row in rows:
            ...
            try:
                ...
                parts = ts.split(" ", 1)
                date_str = parts[0] if len(parts) > 0 else ts
            ...
            writer.writerow({
                ...
                "now_weight":   row.get("initial_weight", ""),
                "req_weight":   row.get("required_weight", ""),
                "final_weight": row.get("final_weight", ""),
                "task_state":   row.get("status", ""),
...
def _build_summary(rows: list, csv_filename: str) -> str:
    """
    Builds the email body with a production summary block.

    IN  transaction : (now_weight - final_weight) < 0  -> final is higher -> weight was added
    OUT transaction : (now_weight - final_weight) >= 0 -> now is higher   -> weight was removed

    Total IN  = sum of final_weight for IN  rows
    Total OUT = sum of final_weight for OUT rows
    """
    total = len(rows)
    ...
    for r in rows:
        try:
            diff = float(r.get("initial_weight") or 0) - float(r.get("final_weight") or 0)
        except (TypeError, ValueError):
            diff = 0.0
        if diff < 0:
            total_in=total_in+(-1*diff)
            in_rows.append(r)       # final > now  ->  IN
        else:
            total_out=total_out+(1 * diff)
            out_rows.append(r)      # now >= final ->  OUT

    #total_in  = sum(float(r.get("final_weight") or 0) for r in in_rows)
    #total_out = sum(float(r.get("final_weight") or 0) for r in out_rows)
    avg_in    = (total_in  / len(in_rows))  if in_rows  else 0.0
    avg_out   = (total_out / len(out_rows)) if out_rows else 0.0
    ...
    lines = [
        ...
        "  IN  Transactions: " + str(len(in_rows)),
        "  OUT Transactions: " + str(len(out_rows)),
        "  Total IN        : " + f"{total_in:.2f} kg",
        "  Total OUT       : " + f"{total_out:.2f} kg",
        "  Avg IN          : " + f"{avg_in:.2f} kg",
        "  Avg OUT         : " + f"{avg_out:.2f} kg",
        sep,
        "",
        "Have a nice day!",
        "",
...
#  MAIN PROCESSING
# ══════════════════════════════════════════════════════════════════
def process_email(subject_or_uid, email_msg_or_sender=None):
    """
    Full pipeline for one email.

    Normal use (called from poll_inbox — pass parsed email object):
        process_email(uid_bytes, parsed_email_message)
    ...
    if isinstance(subject_or_uid, str) and isinstance(email_msg_or_sender, str):
        ...
        # Extract bare email address from strings like "From:NAME@DOMAIN.COM"
        m = re.search(r"[\w.+\-]+@[\w.\-]+", email_msg_or_sender)
        sender  = m.group(0) if m else email_msg_or_sender
        log.info("[TEST MODE] subject=%r  sender=%r", subject, sender)
    else:
        # Normal path — second arg is a parsed email message object
        email_msg = email_msg_or_sender
    ...
    try:
        ...
        body       = _build_summary(rows, csv_fname)
        _send_email(
            to_addr         = sender,
...
if __name__ == "__main__":
    ...
    # ── Test mode ─────────────────────────────────────────────────
    # Run without polling the inbox:
    #   python3 emailAutoSend.py test "sendme 20260501000000 20260502000000" "you@email.com"
    if len(sys.argv) == 4 and sys.argv[1] == "test":
        log.info("Running in TEST MODE — no email will be fetched or sent.")
        process_email(sys.argv[2], sys.argv[3])
        sys.exit(0)
